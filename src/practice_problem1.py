...
class Box(object):
    ...
    def append_string(self, additional_contents):
        ...
        if len(self.contents) + len(additional_contents) > self.volume:
            extra_length = len(self.contents) + len(additional_contents) - self.volume
            for k in range(self.volume - len(self.contents)):
                self.contents = self.contents + additional_contents[k]

            for k in range(len(additional_contents) - extra_length, len(additional_contents)):
    ...
    def shrink(self, new_volume):
        ...
        if new_volume >= len(self.contents):
            ...
            self.volume = new_volume
            new_string = self.contents
            self.contents = ''
            for k in range(len(new_string)):
                if k < self.volume:
                    self.contents = self.contents + new_string[k]
                else:
                    extra = extra + new_string[k]
                    self.discards = self.discards + 1
